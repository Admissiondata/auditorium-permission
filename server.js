...
const localAuditoriums = [
  { id: 1, name: 'Architecture Auditorium', capacity: 300, min_students: 1, approval_1_role: 'head', approval_2_role: 'electrician', approval_3_role: 'principal', approval_4_role: 'maintenance', head_user_id: '', principal_user_id: '', maintenance_user_id: '', electrician_user_id: '', admin_officer_user_id: '', office_superintendent_user_id: '' },
  { id: 2, name: 'Aeronautical Auditorium', capacity: 300, min_students: 1, approval_1_role: 'head', approval_2_role: 'electrician', approval_3_role: 'principal', approval_4_role: 'maintenance', head_user_id: '', principal_user_id: '', maintenance_user_id: '', electrician_user_id: '', admin_officer_user_id: '', office_superintendent_user_id: '' },
  { id: 3, name: 'Main Auditorium', capacity: 500, min_students: 1, approval_1_role: 'head', approval_2_role: 'electrician', approval_3_role: 'principal', approval_4_role: 'maintenance', head_user_id: '', principal_user_id: '', maintenance_user_id: '', electrician_user_id: '', admin_officer_user_id: '', office_superintendent_user_id: '' },
  { id: 4, name: 'Seminar Auditorium', capacity: 250, min_students: 1, approval_1_role: 'head', approval_2_role: 'electrician', approval_3_role: 'principal', approval_4_role: 'maintenance', head_user_id: '', principal_user_id: '', maintenance_user_id: '', electrician_user_id: '', admin_officer_user_id: '', office_superintendent_user_id: '' }
];
const users = [
...
async function ensureAdminAccount() {
  if (!supabase) return;
  try {
    const { data: adminUser, error } = await supabase.from('user_accounts').select('id, role').eq('id', 'admin@svitvasad.ac.in').maybeSingle();
    if (error) return;
    if (!adminUser) {
      await supabase.from('user_accounts').insert({
        id: 'admin@svitvasad.ac.in',
      ...
      console.warn(`Admin role was "${adminUser.role}", restoring to "admin"...`);
      await supabase.from('user_accounts').update({ role: 'admin' }).eq('id', 'admin@svitvasad.ac.in');
    }
  } catch (err) {
...
async function getUsers() {
  ...
  return (data || []).map((u) => {
    if (u.id.toLowerCase() === 'admin@svitvasad.ac.in' && u.role !== 'admin') {
      ensureAdminAccount();
      return { ...u, role: 'admin', is_disabled: false };
    }
    return { ...u, is_disabled: Boolean(u.is_disabled) || localDisabledUsers.has(u.id) };
  });
}
...
async function deleteRoleAssignment(userId, department, role, assignmentId = null) {
  const deleteLocal = () => {
    const index = localRoleAssignments.findIndex((a) => (assignmentId && a.id === Number(assignmentId)) || (a.user_id === userId && (!department || a.department === department) && (!role || a.role === role)));
    if (index >= 0) localRoleAssignments.splice(index, 1);
  };
  if (!supabase) { deleteLocal(); return; }
  try {
    let query = supabase.from('role_assignments').delete();
    if (assignmentId) {
      query = query.eq('id', Number(assignmentId));
    } else {
    ...
    deleteLocal();
    if (e.code !== 'PGRST205' && e.code !== '42P01' && e.code !== '42501') throw e;
  }
}
async function deleteAllRoleAssignments(userId) {
  for (let i = localRoleAssignments.length - 1; i >= 0; i--) {
    if (localRoleAssignments[i].user_id === userId) localRoleAssignments.splice(i, 1);
  }
  if (!supabase) return;
  try {
    const { error } = await supabase.from('role_assignments').delete().eq('user_id', userId);
    if (error && error.code !== 'PGRST205' && error.code !== '42P01' && error.code !== '42501') {
      console.error(`deleteAllRoleAssignments error: ${error.message}`);
    }
  } catch (e) {
    if (e.code !== 'PGRST205' && e.code !== '42P01' && e.code !== '42501') {
      console.error(`deleteAllRoleAssignments exception: ${e.message}`);
    }
  }
}
...
async function approverEmail(role, request, auditorium) {
  ...
  if (role === 'electrician' && auditorium?.electrician_user_id) {
    return auditorium.electrician_user_id;
  }
  if (role === 'admin_officer' && auditorium?.admin_officer_user_id) {
    return auditorium.admin_officer_user_id;
  }
  if (role === 'office_superintendent' && auditorium?.office_superintendent_user_id) {
    return auditorium.office_superintendent_user_id;
  }
...
const forgotStyle = `.login-layout{display:grid;grid-template-columns:1fr 1fr;gap:0;min-height:100vh}.login-left{background:var(--ink);color:var(--paper);padding:60px 56px;display:flex;flex-direction:column;justify-content:center;position:relative}.login-left .brand{position:absolute;top:40px;left:56px}.login-left .brand h2{font-size:14px;font-weight:700;letter-spacing:.14em;text-transform:uppercase;color:rgba(255,255,255,.5);margin:0}.login-left .brand p{font-size:11px;color:rgba(255,255,255,.3);margin:4px 0 0;letter-spacing:.06em}.login-left h1{font-size:clamp(36px,5vw,56px);font-weight:700;margin:0 0 20px;letter-spacing:.02em;line-height:1.05}.login-left .rule{width:50px;height:3px;background:var(--lime);margin:0 0 28px;border-radius:2px}.login-left p{font-size:15px;line-height:1.6;color:var(--muted);margin:0 0 15px}.login-right{padding:60px 56px;display:flex;flex-direction:column;justify-content:center}.login-right .eyebrow{color:var(--orange);margin-bottom:14px;font-size:11px;text-transform:uppercase;letter-spacing:.14em;font-weight:600}.login-right h2{font-size:38px;font-weight:400;margin:0 0 40px;line-height:1.1;letter-spacing:-.02em}.login-right h2 em{font-style:italic;color:var(--orange)}.login-form{max-width:400px}.login-form label{display:flex;flex-direction:column;gap:8px;font-size:11px;text-transform:uppercase;letter-spacing:.08em;color:var(--muted);font-weight:500;margin-bottom:28px}.login-form input{font-size:16px;padding:12px 0 14px;border:0;border-bottom:2px solid var(--line);border-radius:0;background:transparent}.login-form input:focus{border-color:var(--ink);box-shadow:none}.login-form button{width:100%;margin-top:12px;padding:15px;font-size:14px;border-radius:var(--radius-sm)}.login-help{font-size:12px;line-height:1.5;color:var(--muted);margin-top:24px}.login-alert{background:rgba(233,119,66,.1);border:1px solid var(--orange);color:var(--ink);padding:14px;border-radius:var(--radius-sm);margin-bottom:20px;font-size:14px;line-height:1.5}@media(max-width:860px){.login-layout{grid-template-columns:1fr}.login-left{padding:40px 28px}.login-left .brand{position:static;margin-bottom:40px}.login-right{padding:40px 28px}}`;
app.get('/forgot-password', async (req, res) => {
  const heading = await collegeHeading();
  const page = `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Forget Password | SVIT Vasad</title><link rel="stylesheet" href="/styles.css"><style>${forgotStyle}</style></head><body><main class="shell"><div class="login-layout"><section class="login-left"><div class="brand"><h2>SVIT Vasad</h2><p>Campus Management System</p></div><h1>Reset<br>password.</h1><div class="rule"></div><p>Enter your registered email address and we will send you instructions to reset your password.</p><p style="margin-top:20px">Contact the administrator if you need immediate access.</p></section><section class="login-right"><p class="eyebrow">Forget password</p><h2>Enter your<br><em>email address.</em></h2><form class="login-form" action="/forgot-password" method="post"><label>Email ID<input name="user_id" type="email" autocomplete="username" placeholder="name@svitvasad.ac.in" required></label><button type="submit">Send reset instructions <span>↗</span></button><p class="login-help"><a href="/login">← Back to sign in</a></p></form></section></div><footer><span>Auditorium Registration Application</span><span>Admissiondata / 2026</span></footer></main></body></html>`;
  res.send(page);
});
...
const renderLoginRoleSelect = (res, user, assignments, error) => {
  ...
  res.send(page
    ...
    .replace(/<p class="login-help">.*?<\/p>/, '<p class="login-help">This email has multiple roles. Choose one to continue.</p>')
...
app.post('/login', async (req, res) => {
  ...
  const inputId = String(req.body.user_id || '').trim().toLowerCase();
  const inputPassword = String(req.body.password || '');
  const user = allUsers.find((candidate) => {
    const candidateId = candidate.id.toLowerCase().trim();
    const idMatch = candidateId === inputId || (inputId === 'admin' && candidateId === 'admin@svitvasad.ac.in');
    return idMatch && candidate.password === inputPassword;
  });
...
app.get('/dashboard', requireLogin, async (req, res) => {
  ...
  res.send(`<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Dashboard | SVIT Vasad</title><link rel="stylesheet" href="/styles.css"><style>
    .dash-head{display:flex;justify-content:space-between;align-items:center;gap:16px;flex-wrap:wrap;padding:24px 0;border-bottom:2px solid var(--ink)}
    .dash-nav{display:flex;align-items:center;gap:14px;flex-wrap:wrap}
    .nav-link{color:var(--ink);text-decoration:none;font-weight:600;font-size:13px;position:relative;padding:6px 12px;border-radius:var(--radius-sm);transition:all var(--transition)}
    .nav-link:hover{background:rgba(15,26,21,0.05)}
    .notif-badge{position:absolute;top:-6px;right:-6px;background:var(--orange);color:#fff;border-radius:10px;font-size:10px;padding:2px 6px;font-weight:700}
    .nav-user{color:var(--ink);font-weight:600;font-size:13px}
    .nav-role{text-transform:uppercase;font-size:10px;font-weight:600;letter-spacing:0.08em;border:1px solid var(--line);padding:4px 10px;border-radius:20px;color:var(--muted)}
    .portal-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:16px;margin-top:28px}
    .portal-card{display:flex;gap:16px;align-items:flex-start;padding:24px;border:1px solid var(--line);border-radius:var(--radius-md);text-decoration:none;color:var(--ink);transition:all var(--transition);background:var(--white)}
    .portal-card:hover{transform:translateY(-2px);box-shadow:var(--shadow-lg);border-color:var(--accent)}
    .portal-icon{font-size:28px;line-height:1;width:48px;height:48px;display:flex;align-items:center;justify-content:center;background:rgba(15,26,21,0.03);border-radius:var(--radius-sm);flex-shrink:0}
    .portal-body h3{margin:0 0 6px;font-size:17px;font-weight:600}
    .portal-stat{display:block;color:var(--muted);font-size:13px;margin-bottom:10px;line-height:1.4}
    .portal-open{display:inline-block;font-size:12px;font-weight:600;color:var(--accent)}
    .pending-block{border:1px solid var(--line);border-left:4px solid var(--orange);border-radius:var(--radius-md);padding:20px 24px;margin-top:28px;background:var(--white)}
    .pending-title{font-size:11px;line-height:1.4;letter-spacing:0.1em;text-transform:uppercase;color:var(--muted);margin-bottom:14px;font-weight:600}
    .pending-tabs{display:flex;gap:8px;flex-wrap:wrap}
    .pending-tab{position:relative;font-size:13px;font-weight:600;text-decoration:none;color:var(--ink);border:1px solid var(--line);padding:10px 16px;border-radius:var(--radius-sm);display:inline-flex;gap:8px;align-items:center;background:var(--white);transition:all var(--transition)}
    .pending-tab:hover{border-color:var(--orange);color:var(--orange)}
    .pending-badge{background:var(--orange);color:#fff;border-radius:20px;font-size:11px;padding:3px 10px;font-weight:700}
    .muted{opacity:.55}
    @media(max-width:600px){.dash-head{flex-direction:column;align-items:flex-start}.portal-grid{grid-template-columns:1fr}}
  </style></head><body><main class="shell panel">
    <div class="college-heading" style="padding:16px 0 10px"><div style="font-size:10px;letter-spacing:0.18em;color:var(--muted);font-weight:600">SARDAR VALLABHBHAI PATEL INSTITUTE OF TECHNOLOGY · VASAD CAMPUS</div></div>
    <header class="dash-head"><h1 style="margin:0;font-size:28px;font-weight:600">Campus Administration Portal</h1>${headerNav}</header>
...
const roleNames = {
  ...
  head: 'Department head', maintenance: 'Maintenance officer', electrician: 'Electrician', principal: 'Principal', office_superintendent: 'Office Superintendent', higher_authority: 'Higher authority', work_done: 'Work Inspector'
};

const roleOptionsForUser = (selected) => {
  const presets = ['admin', 'sub_admin', 'admin_officer', 'purchase_officer', 'purchase_clerk', 'chairman', 'department_user', 'head', 'maintenance', 'electrician', 'principal', 'office_superintendent', 'higher_authority', 'work_done'];
...
const userRowTemplate = (candidate, currentUser, departmentOptions, prefix = 'all') => {
  const formId = `user-form-${prefix}-${encodeURIComponent(candidate.id).replace(/[^a-z0-9]/gi, '_')}_${(candidate.department || '').replace(/[^a-z0-9]/gi, '_')}_${candidate.role || ''}`;
  const isAssignment = Boolean(candidate._assignment_id);
  const canDelete = candidate.id !== currentUser.id && candidate.id !== 'admin@svitvasad.ac.in';
  ...
  const isPresetRole = ['admin', 'sub_admin', 'admin_officer', 'purchase_officer', 'purchase_clerk', 'chairman', 'department_user', 'head', 'maintenance', 'electrician', 'principal', 'office_superintendent', 'higher_authority', 'work_done'].includes(candidate.role);
  const customRole = !isPresetRole ? candidate.role : '';

  if (isAssignment) {
    return `<tr><td><form id="${formId}" action="/admin/users/${encodeURIComponent(candidate.id)}/assignment" method="post"><input type="hidden" name="tab" value="${escapeHtml(prefix)}"><input type="hidden" name="assignment_id" value="${escapeHtml(String(candidate._assignment_id || ''))}"><input type="hidden" name="orig_department" value="${escapeHtml(candidate.department)}"><input type="hidden" name="orig_role" value="${escapeHtml(candidate.role)}"></form><strong class="email">${escapeHtml(candidate.id)}</strong>${multiRoleNote}</td><td><input form="${formId}" name="name" value="${escapeHtml(candidate.name)}" aria-label="Name" required></td><td><select form="${formId}" name="department" aria-label="Department" required><option value="">Select department</option>${departmentOptions(candidate.department)}</select></td><td><select form="${formId}" name="role" aria-label="Role">${roleOptionsForUser(candidate.role)}</select><input form="${formId}" name="custom_role" value="${escapeHtml(customRole)}" placeholder="Custom role name" style="display:none" aria-label="Custom role"></td><td>${statusBadge}</td><td class="user-actions"><button form="${formId}" class="small-button" type="submit" onclick="return confirm('Save changes to this role assignment?')">Save</button> <form action="/admin/users/${encodeURIComponent(candidate.id)}/assignment/delete" method="post" style="display:inline" onsubmit="return confirm('Remove role (${escapeHtml(candidate.role)}) for ${escapeHtml(candidate.id)} from this workspace?')"><input type="hidden" name="tab" value="${escapeHtml(prefix)}"><input type="hidden" name="assignment_id" value="${escapeHtml(String(candidate._assignment_id || ''))}"><input type="hidden" name="department" value="${escapeHtml(candidate.department)}"><input type="hidden" name="role" value="${escapeHtml(candidate.role)}"><button class="small-button reject-button" type="submit">Delete role</button></form>${canDelete ? ` <form action="/admin/users/${encodeURIComponent(candidate.id)}/delete" method="post" style="display:inline" onsubmit="return confirm('Delete this entire user account (${escapeHtml(candidate.id)}) and all their roles permanently?')"><input type="hidden" name="tab" value="${escapeHtml(prefix)}"><button class="small-button" type="submit">Delete user</button></form>` : ''}</td></tr>`;
  }

  return `<tr><td><form id="${formId}" action="/admin/users/${encodeURIComponent(candidate.id)}" method="post"><input type="hidden" name="tab" value="${escapeHtml(prefix)}"><input name="id" type="email" value="${escapeHtml(candidate.id)}" aria-label="Email" required></form></td><td><input form="${formId}" name="name" value="${escapeHtml(candidate.name)}" aria-label="Name" required></td><td><select form="${formId}" name="department" aria-label="Department" required><option value="">Select department</option>${departmentOptions(candidate.department)}</select></td><td><input form="${formId}" name="phone" value="${escapeHtml(candidate.phone || '')}" placeholder="WhatsApp number" aria-label="WhatsApp number"><select form="${formId}" name="role" aria-label="Role">${roleOptionsForUser(candidate.role)}</select><input form="${formId}" name="custom_role" value="${escapeHtml(customRole)}" placeholder="Custom role name" style="display:none" aria-label="Custom role"></td><td>${statusBadge}</td><td class="user-actions"><input form="${formId}" name="password" type="password" placeholder="New password (optional)" aria-label="New password"><button form="${formId}" class="small-button" type="submit" onclick="return confirm('Are you sure you want to save changes to this user?')">Save</button> ${canDelete ? `<form action="/admin/users/${encodeURIComponent(candidate.id)}/toggle" method="post" style="display:inline" onsubmit="return confirm('${isDisabled ? 'Enable this user account?' : 'Disable this user account? The user will not be able to log in.'}')"><input type="hidden" name="tab" value="${escapeHtml(prefix)}"><button class="small-button" type="submit">${isDisabled ? 'Enable' : 'Disable'}</button></form> <form action="/admin/users/${encodeURIComponent(candidate.id)}/delete" method="post" style="display:inline" onsubmit="return confirm('Are you sure you want to delete this user permanently?')"><input type="hidden" name="tab" value="${escapeHtml(prefix)}"><button class="small-button reject-button" type="submit">Delete</button></form>` : '<span class="small-copy">Signed in</span>'}</td></tr>`;
};
...
app.post('/admin/auditoriums', requireLogin, async (req, res) => {
  ...
  const values = { name, capacity, min_students: 1, head_user_id: '', principal_user_id: '', maintenance_user_id: '', electrician_user_id: '', admin_officer_user_id: '', office_superintendent_user_id: '' };
...
app.get('/admin/auditoriums/manage', requireLogin, async (req, res) => {
  ...
  const assignableRole = new Set(['head', 'electrician', 'maintenance', 'principal', 'admin_officer', 'office_superintendent']);
  const assignableUsersById = new Map();
  allUsers.filter((u) => assignableRole.has(u.role) || u.role === 'admin' || u.role === 'sub_admin')
    .forEach((u) => assignableUsersById.set(String(u.id).toLowerCase(), { id: u.id, name: u.name, department: u.department, role: u.role }));
  roleAssignments.filter((a) => assignableRole.has(a.role))
    .forEach((a) => { const key = String(a.user_id).toLowerCase(); if (!assignableUsersById.has(key)) assignableUsersById.set(key, { id: a.user_id, name: a.name || a.user_id, department: a.department || '', role: a.role }); });
  departmentRows.filter((d) => d.head_user_id)
    .forEach((d) => { const key = String(d.head_user_id).toLowerCase(); if (!assignableUsersById.has(key)) assignableUsersById.set(key, { id: d.head_user_id, name: d.head_user_id, department: d.name || '', role: 'head' }); });
  const assignableUsers = [...assignableUsersById.values()];
  const audAuthorityRoles = [
    { role: 'head', label: 'Department Head' },
    { role: 'electrician', label: 'Electrician' },
    { role: 'maintenance', label: 'Maintenance' },
    { role: 'principal', label: 'Principal' },
    { role: 'admin_officer', label: 'Admin Officer' },
    { role: 'office_superintendent', label: 'Office Superintendent' }
  ];
  let stageCount = 4;
  auditoriums.forEach((a) => { const c = approvalRoles(a).length; if (c > stageCount) stageCount = c; });
  const stageSelect = (selected, nameAttr) => `<select name="${nameAttr}">${['none', 'head', 'electrician', 'principal', 'maintenance', 'chairman', 'admin_officer', 'office_superintendent', 'higher_authority', 'purchase_officer', 'work_done', 'department_user', 'sub_admin', 'admin'].map((role) => `<option value="${role}"${role === (selected || 'none') ? ' selected' : ''}>${role === 'none' ? '— None' : escapeHtml(roleNames[role] || role)}</option>`).join('')}</select>`;
  const roleUserSelect = (role, selectedId, nameAttr, placeholder) => {
    const pool = assignableUsers.filter((u) => u.role === role || u.role === 'admin' || u.role === 'sub_admin');
    const options = pool.map((u) => `<option value="${escapeHtml(u.id)}"${u.id === selectedId ? ' selected' : ''}>${escapeHtml(u.name)} (${escapeHtml(u.department)})</option>`).join('');
    const preserved = selectedId && !pool.some((u) => u.id === selectedId) ? `<option value="${escapeHtml(selectedId)}" selected>${escapeHtml(selectedId)}</option>` : '';
    return `<select name="${nameAttr}" style="min-width:180px"><option value="">— ${placeholder} —</option>${preserved}${options}</select>`;
  };
  const stageHeaders = (() => {
    let html = '';
...
app.post('/admin/auditoriums/manage', requireLogin, async (req, res) => {
  ...
  try {
    for (let ri = 0; ri < ids.length; ri++) {
      ...
      const values = { name: String(names[ri] || current.name).trim(), min_students: current.min_students || 1, capacity: current.capacity || 300, head_user_id: current.head_user_id || '', principal_user_id: current.principal_user_id || '', maintenance_user_id: current.maintenance_user_id || '', electrician_user_id: current.electrician_user_id || '', admin_officer_user_id: current.admin_officer_user_id || '', office_superintendent_user_id: current.office_superintendent_user_id || '' };
      applyLegacyRoles(values, roles);
      if (supabase) {
...
app.post('/admin/auditoriums/heads', requireLogin, async (req, res) => {
  ...
  const authorityRoles = ['head', 'electrician', 'maintenance', 'principal', 'admin_officer', 'office_superintendent'];
  const collectAuthority = (ri) => {
    const values = {};
    authorityRoles.forEach((role) => { values[`${role}_user_id`] = String(req.body[`${role}_user_id_${ri}`] || '').trim() || ''; });
    return values;
  };
  ...
  try {
    for (let ri = 0; ri < audIds.length; ri++) {
      const id = audIds[ri];
      const values = collectAuthority(ri);
      if (supabase) {
        const { error } = await supabase.from('auditoriums').update(values).eq('id', String(id));
        if (error) {
          const missingColumn = authorityRoles.find((role) => error.message && error.message.includes(`${role}_user_id`));
  ...
  if (columnMissing) {
    return res.redirect('/admin/auditoriums/manage?msg=' + encodeURIComponent('One or more authority columns are missing. Run migrate-auditorium-approvals.sql in Supabase SQL Editor, then try again.'));
  }
  res.redirect('/admin/auditoriums/manage');
});
...
app.get('/admin/departments', requireLogin, async (req, res) => {
  ...
  const roleSelectOptions = ['admin', 'sub_admin', 'admin_officer', 'purchase_officer', 'purchase_clerk', 'chairman', 'department_user', 'head', 'maintenance', 'electrician', 'principal', 'office_superintendent', 'higher_authority', 'work_done'].map((role) => `<option value="${role}">${roleNames[role]}</option>`).join('');
...
app.get('/admin/users/auditorium', requireLogin, async (req, res) => {
  if (!isAdmin(req.session.user)) return res.status(403).send('Admin access required.');
  await moduleUsersPage(req, res, { title: 'Auditorium', moduleKey: 'auditorium', icon: '🎓', roles: ['head', 'department_user', 'admin_officer', 'office_superintendent'], description: 'Auditorium request and approval users.' });
});
...
function requestRow(request, user, auditoriumConfigs) {
  ...
  const auditorium = auditoriumConfigs.find((a) => a.name === request.auditorium) || {};
  const headInfo = auditorium.head_user_id ? `<small style="color:var(--orange);font-size:11px;display:block">Head: <span class="email">${escapeHtml(auditorium.head_user_id)}</span></small>` : '';
...
function requestStatus(request, auditoriumConfigs) {
  ...
  const stepRoleNames = { head: 'Department head', electrician: 'Electrician', principal: 'Principal', maintenance: 'Maintenance officer', chairman: 'Chairman', admin_officer: 'Admin officer', office_superintendent: 'Office Superintendent', higher_authority: 'Higher authority', purchase_officer: 'Purchase officer', work_done: 'Work Inspector', department_user: 'Department staff' };
  ...
  if (transition.role === 'principal' && auditorium.principal_user_id) pendingLabel += ` (<span class="email">${escapeHtml(auditorium.principal_user_id)}</span>)`;
  if (transition.role === 'maintenance' && auditorium.maintenance_user_id) pendingLabel += ` (<span class="email">${escapeHtml(auditorium.maintenance_user_id)}</span>)`;
  if (transition.role === 'electrician' && auditorium.electrician_user_id) pendingLabel += ` (<span class="email">${escapeHtml(auditorium.electrician_user_id)}</span>)`;
  if (transition.role === 'admin_officer' && auditorium.admin_officer_user_id) pendingLabel += ` (<span class="email">${escapeHtml(auditorium.admin_officer_user_id)}</span>)`;
  if (transition.role === 'office_superintendent' && auditorium.office_superintendent_user_id) pendingLabel += ` (<span class="email">${escapeHtml(auditorium.office_superintendent_user_id)}</span>)`;
...
function requestNeedsUserApproval(user, request, auditoriumConfigs) {
  ...
  if (transition.role === 'office_superintendent' && auditorium.office_superintendent_user_id && user.id !== auditorium.office_superintendent_user_id) return false;
  return true;
}

function requestVisibleToUser(user, request, auditoriumConfigs) {
  if (isAdmin(user) || ['principal', 'maintenance', 'electrician', 'admin_officer', 'office_superintendent', 'chairman', 'higher_authority', 'purchase_officer'].includes(user.role)) return true;
  if (user.role === 'head') {
    const auditorium = auditoriumConfigs.find((candidate) => candidate.name === request.auditorium) || {};
    const auditoriumHead = auditorium.head_user_id;
...
app.post('/admin/users', requireLogin, async (req, res) => {
  ...
  if (email === 'admin@svitvasad.ac.in') role = 'admin';
    ...
    for (const [rowIndex, row] of rows.entries()) {
      ...
      const reason = missing.length && !(missing.length === 1 && missing[0] === 'password' && existing)
        ...
        : !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email) ? 'invalid Email'
          : !['admin', 'sub_admin', 'department_user', 'head', 'maintenance', 'electrician', 'principal', 'admin_officer', 'purchase_officer', 'purchase_clerk', 'chairman', 'higher_authority', 'work_done', 'office_superintendent'].includes(role) ? `invalid Role "${String(row.role).trim()}"`
...
app.post('/admin/users/:id/delete', requireLogin, async (req, res) => {
  ...
  if (req.params.id === 'admin@svitvasad.ac.in') return res.status(400).send('The primary administrator account cannot be deleted.');
  const currentUsers = await getUsers();
  const roleAssignments = (await getRoleAssignments()) || [];
  const existing = currentUsers.find((candidate) => candidate.id === req.params.id);
  const hasAssignment = roleAssignments.some((a) => a.user_id === req.params.id);
  if (!existing && !hasAssignment) return res.status(404).send('User not found.');
  ...
  await deleteAllRoleAssignments(req.params.id);
...
app.post('/admin/users/:id/toggle', requireLogin, async (req, res) => {
  ...
  if (req.params.id === 'admin@svitvasad.ac.in') return res.status(400).send('The primary administrator account cannot be disabled.');
  const currentUsers = await getUsers();
  const existing = currentUsers.find((candidate) => candidate.id === req.params.id);
...
const workflowRoleOptions = (selected) => ['head', 'admin_officer', 'purchase_officer', 'purchase_clerk', 'principal', 'chairman', 'electrician', 'maintenance', 'office_superintendent', 'higher_authority'].map((r) => `<option value="${r}"${r === selected ? ' selected' : ''}>${moduleRoleLabel(r)}</option>`).join('');
...
const permissionRoles = ['head', 'department_user', 'purchase_officer', 'purchase_clerk', 'admin_officer', 'office_superintendent', 'principal', 'chairman', 'maintenance', 'electrician', 'higher_authority'];
...
app.post('/admin/requests/:id/approve', requireLogin, async (req, res) => {
  ...
  if (user.role === 'office_superintendent' && auditorium.office_superintendent_user_id && user.id !== auditorium.office_superintendent_user_id) return res.status(403).send('This request is assigned to another office superintendent.');
...
app.get('/admin/maintenance', requireLogin, async (req, res) => {
  ...
  const rows = allRequests.length ? sortPendingFirst(allRequests).map((req) => {
    ...
    return `<tr>
      ...
      <td>${escapeHtml(req.description)}<small>${escapeHtml(req.reporter_name)} · ${escapeHtml(req.department)}</small><small>${escapeHtml(req.reporter_mobile)} · <span class="email">${escapeHtml(req.reporter_email)}</span></small></td>
...
const defaultModulePermissions = [
  ['auditorium', 'admin', 'manage'], ['auditorium', 'head', 'approve'], ['auditorium', 'department_user', 'request'], ['auditorium', 'sub_admin', 'manage'], ['auditorium', 'principal', 'approve'], ['auditorium', 'admin_officer', 'approve'], ['auditorium', 'office_superintendent', 'approve'],
...
app.get('/admin/purchase', requireLogin, async (req, res) => {
  ...
  for (const req of sortPendingFirst(allRequests)) {
    ...
    rowChunks.push(`<tr>
      ...
      <td><span class="priority-badge" style="background:${priorityBg[req.priority] || '#f0f0f0'};color:${priorityColors[req.priority] || 'var(--ink)'}">${escapeHtml(req.priority.toUpperCase())}</span></td>
      <td>${escapeHtml(req.requester_name)}<small>${escapeHtml(req.requester_mobile)}</small><small><span class="email">${escapeHtml(req.requester_email)}</span></small></td>
      <td>${createdDate}<small>${createdTime}</small></td>
...
app.get('/admin/reports', requireLogin, async (req, res) => {
  ...
  const bookingStatuses = ['pending', 'first_approved', 'second_approved', 'third_approved', 'fourth_approved', 'approved', 'rejected'];
...
app.get('/admin/fees', requireLogin, async (req, res) => {
  ...
  const studentRows = students.length ? students.map((s) => `<tr><td>${escapeHtml(s.enrollment_no)}</td><td>${escapeHtml(s.name)}</td><td>${escapeHtml(s.department)}</td><td>₹${Number(s.total_fees || 0).toFixed(2)}</td><td><span class="email">${escapeHtml(s.email || '-')}</span></td></tr>`).join('') : '<tr><td colspan="5">No students uploaded yet.</td></tr>';
